...
import type { FileSummaryEntry } from '../utils/index-lookup.js';
  ...
  return tool({
    ...
    execute: async ({ path: filePath, offset, limit }) => {
      ...
      try {
        ...
        // B: Inject pre-built index summary when available so the agent gets
        // semantic context (purpose, exports, function list) without an extra
        // semantic_search call.
        if (getIndexSummary) {
          const summary = getIndexSummary(relPath);
          if (summary) {
            response['indexSummary'] = {
              purpose: summary.purpose,
              exports: summary.exports,
              // Compact: "functionName: one-sentence description"
              functions: summary.functions.map((f) => `${f.name}: ${f.description}`),
            };
          }
        }
...
/** Backward-compatible export for callers that don't need the index enrichment. */
